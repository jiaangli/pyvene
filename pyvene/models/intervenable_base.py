...
class BaseModel(nn.Module):
    ...
    def __init__(self, config, model, backend, **kwargs):
        ...
        for i, representation in enumerate(config.representations):
            ...
            if representation.intervention is not None:
                ...
                component_dim = get_dimension_by_component(
                    get_internal_model_type(model), model.config, representation.component
                )
            ...
            module_hook = get_module_hook(model, representation, backend)
        ...
        if self.config.sorted_keys is not None:
            logging.warn("The key is provided in the config. Assuming this is loaded from a pretrained module.")
        if self.config.sorted_keys is not None or "intervenables_sort_fn" not in kwargs:
            ...
            self.sorted_keys = kwargs["intervenables_sort_fn"](model, self.representations)
        ...
        if _any_group_key:
            ...
            for i in range(len(_validate_group_keys) - 1):
                if _validate_group_keys[i] > _validate_group_keys[i + 1]:
                    logging.info(f"This is not a valid group key order: {_validate_group_keys}")
                    raise ValueError(
                        "Must be ascending order. Interventions would be performed in order within group as well"
                    )
        else:
    ...
    def set_temperature(self, temp: torch.Tensor):
        ...
        """
        for k, v in self.interventions.items():
            if isinstance(v[0], BoundlessRotatedSpaceIntervention) or isinstance(v[0], SigmoidMaskIntervention):
    ...
    def _input_validation(
        ...
        if sources is not None and not (len(sources) == 1 and sources[0] == None):
            if len(sources) != len(self._intervention_group):
                raise ValueError(
                    f"Source length {len(sources)} is not equal to intervention length {len(self._intervention_group)}."
                )
        ...
        if not self.is_model_stateless and activations_sources is not None:
            for _, v in activations_sources.items():
                if isinstance(v, list) and isinstance(v[0], tuple) and isinstance(v[0][1], list) != True:
                    raise ValueError(f"Stateful models need nested activations. See our documentions.")

    def _gather_intervention_output(self, output, representations_key, unit_locations) -> torch.Tensor:
        """
        ...
        if (
            ...
            and self._intervention_reverse_link[representations_key] in self.hot_activations
        ):
            # hot gather
            ...
            selected_output = self.hot_activations[self._intervention_reverse_link[representations_key]]
        else:
    ...
    def _broadcast_source_representations(self, source_representations):
        ...
        if isinstance(source_representations, dict) or source_representations is None:
            ...
            raise ValueError("Accept input type for source_representations is [Dict, List, torch.Tensor]")
...
class IntervenableNdifModel(BaseModel):
    ...
    def __init__(self, config, model, **kwargs):
        ...
        logging.warning(f"We currently have very limited intervention support for ndif backend.")

    def save(self, save_directory, save_to_hf_hub=False, hf_repo_name="my-awesome-model"):
        pass
    ...
    def _intervention_getter(
        ...
        for key_i, key in enumerate(keys):
            ...
            if isinstance(intervention, SkipIntervention):
                ...
                selected_output = self._gather_intervention_output(output, key, unit_locations[key_i])
    ...
    def _intervention_setter(
        ...
        for key_i, key in enumerate(keys):
            ...
            selected_output = self._gather_intervention_output(output, key, unit_locations_base[key_i])
                ...
                for key in keys:
                    ...
                    if (
                        ...
                        self._intervention_setter(
                            ...
                            [subspaces[self.sorted_keys.index(key)]] if subspaces is not None else None,
                        )
    ...
    def forward(
        ...
        if sources is None and activations_sources is None and unit_locations is None and len(self.interventions) == 0:
            # ndif backend call
        ...
        if return_dict:
            return IntervenableModelOutput(
                original_outputs=base_outputs, intervened_outputs=counterfactual_outputs, collected_activations=None
            )
...
class IntervenableModel(BaseModel):
    ...
    def save(self, save_directory, save_to_hf_hub=False, hf_repo_name="my-awesome-model", include_model=False):
        """
        ...
        for k, v in self.interventions.items():
            ...
            if isinstance(intervention, TrainableIntervention) or intervention.source_representation is not None:
    ...
    @staticmethod
    def load(load_directory, model, local_directory=None, from_huggingface_hub=False, include_model=False):
        """
        Load interventions from disk or hub
        """
        ...
        for i, (k, v) in enumerate(intervenable.interventions.items()):
            ...
            if (
                ...
                try:
                    intervention.register_buffer("source_representation", saved_state_dict["source_representation"])
                except:
        ...
        for key_i, key in enumerate(keys):
            ...
            def hook_callback(model, args, kwargs, output=None):
                ...
                if self.is_model_stateless:
                    ...
                    self.activations.setdefault(key, []).append((selected_output, state_select_flag))
                # set version for stateful models
                self._intervention_state[key].inc_getter_version()
    ...
    def _reconcile_stateful_cached_activations(
        ...
        for unit_location in intervening_unit_locations:
            if self._intervention_state[key].setter_version() in unit_location:
                ...
                state_select_flag += [False]
        state_select_flag = torch.tensor(state_select_flag).bool().to(intervening_activations.device)
    ...
    def train_alignment(
        ...
        **kwargs,
        ...
        gradient_accumulation_steps = (
            kwargs["gradient_accumulation_steps"] if "gradient_accumulation_steps" in kwargs else 1
        )
        ...
        optimizer = kwargs["optimizer"] if "optimizer" in kwargs else optim.Adam(optimizer_params, lr=lr)
        scheduler = (
            kwargs["scheduler"]
            if "scheduler" in kwargs
            else get_linear_schedule_with_warmup(optimizer, num_warmup_steps=warm_up_steps, num_training_steps=t_total)
        )
        ...
        for epoch in epoch_iterator:
            step_iterator = tqdm(
                enumerate(train_dataloader), desc="Step", total=len(train_dataloader), leave=False, position=1
            )
            for step, inputs in step_iterator:
                ...
                unit_location_dict = {"sources->base": inputs["sources->base"].squeeze(1)[0].item()}
                ...
                eval_metrics = compute_metrics([counterfactual_outputs.logits], [inputs["labels"]])
    ...
    def eval_alignment(
        ...
        **kwargs,
        ...
        results = {k: weighted_average(v, all_num_examples) for k, v in all_metrics.items()}
